...
async def chat(
    ...
    # 1. Try to match and return exact PYQ on demand.
    # pyq_result is now a structured dict (not a markdown string) so the
    # frontend can render tappable file items directly. Shape is one of:
    #   {"type": "results",  "code", "semester", "message", "files": [...]}
    #   {"type": "clarify",  "message", "options": [...]}
    #   {"type": "empty",    "code", "message"}
    pyq_result = await get_pyq_response(req.message)
    if pyq_result is not None:
        async def pyq_stream():
            yield f"data: {json.dumps({'response': pyq_result})}\n\n"
